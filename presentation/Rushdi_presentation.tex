...
%	TITLE PAGE
%-------------------------------------------------------
\title[MSc Presentation]{Unified Analytical Overview \\ with Numerical Simulation \\ of the Target-Attacker-Defender Problem} % The short title appears at the bottom of every slide, the full title is only on the title page

\author{Mostafa Ali Rushdi} % Your name
...
\begin{itemize}
\item This thesis deals with an important pursuit-evasion problem that involves three agents, the Target the Attacker and the Defender. The Attacker missile pursues a Target aircraft that is being helped by a Defender missile which tries to intercept the Attacker before it reaches the Target. 
\item A differential game arises in which a team is formed by the Target and the Defender which cooperate to maximize the separation between the Target and the point where the Defender intercepts the Attacker, while the Attacker tries to minimize this separation. 
\item This thesis offers a unified analytic treatment of the aforementioned problem based on the construction of two Apollonius circles.
...
\begin{frame}
\frametitle{Solution}
The treatment includes all possibilities of the ratio between the speeds of the Attacker and Defender. A criticality condition is derived from which two important entities are obtained, namely: 
\begin{itemize}
\item the critical Target speed normalized w.r.t. the Attacker's speed 
