...
main()
{
	int step,dint,i;
	float *z;
	float delay,vm,xncg,alt,a,diam,fr,xl,ctw,crw,hw,ctt;
	float crt,ht,xn,xcg,xhl,wgt,xkr,wact,zact,rho;
	float swing,stail,sref,xlp,splan,xcpn,an,ab,xcpb;
	float xcpw,xmach,xiyy,tmp1,tmp2,tmp3;
	float tmp4,b,q,p1,y1,y2,y3,y4,y5,y6,p2,p3,alftr,deltr;
	float cna,cnd,cmap,cma,cmd,xma,xmd,za,zd,wz,waf,zaf;
	float t,h,s,eold,edold,delold,deldold,edd,deldd,delcp;
	float xk1,xk2,ta,xk3,xkdc,e,ed,deld,del,delc;
	float xnl,thd;
	FILE *fptr;
	fptr=fopen("DATFIL.TXT","w");
	z=(float*)malloc(20003*sizeof(float));
	delay=.015;
	vm=3000.;
	xncg=10.;
	alt=0.;
