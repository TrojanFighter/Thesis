...
#include <stdlib.h>
#include <math.h>
void lambert(double xm,double ym,double tgolam,double xtf,double ytf,
...
void predict (double tf,double xt,double yt,double x1t,double y1t,
		double *xtf,double *ytf);
main()
{
	int step;
	double xlongmdeg,xlongtdeg,altnmtic,altnmmic,tf,gamdegt,h,a;
	double gm,pi,degrad,xnp,prederr,gamt,distnmt,phit,altt,altm;
	double r0t,top,temp,bot,vt,xlongm,xlongt,x1t,y1t,xtf,ytf;
	double s,scount,xm,ym,xt,yt,xfirstt,yfirstt,t;
	double x1m,y1m,rtm1,rtm2,rtm,vtm1,vtm2,vc,delv,tgo,xoldt,yoldt;
	double x1oldt,y1oldt,xoldm,yoldm,x1oldm,y1oldm,delvoldm;
	double xdt,ydt,x1dt,y1dt,xdm,ydm,x1dm,y1dm,delvd;
	double xnmt,ynmt,xnmm,ynmm,altnmt,altnmm,tembott;
	double xlam,xlamd,xnc,am1,am2,tembotm,tgolam,vrxm,vrym;
	double delvold,distnmm;
	FILE *fptr;
	...
	yfirstt=yt;
	t=0.;
	predict (tf,xt,yt,x1t,y1t,&xtf,&ytf);
	ytf=ytf+prederr;
	tgolam=tf;
	lambert(xm,ym,tgolam,xtf,ytf,&vrxm,&vrym,xlongm,xlongt);
	x1m=vrxm;
	y1m=vrym;
	rtm1=xt-xm;
	rtm2=yt-ym;
...
L55:	xt=(xoldt+xt)/2+.5*h*xdt;
	...
	printf("%10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n",t,distnmt
			,altnmt,distnmm,altnmm,xnc,delv);
	fprintf(fptr,"%10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n",t,distnmt
			,altnmt,distnmm,altnmm,xnc,delv);
 	goto L10;
L200:
...
{
	int step;
	double h,a,g,t,x,y,x1,y1,xold,yold,x1old,y1old,xd,yd,x1d,y1d;
	double tembot,gm;
	h=.01;
	a=2.0926e7;
	gm=1.4077e16;
