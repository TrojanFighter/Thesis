...
%%%%%%%%%%%%%%%%%%%%%%
%If dE.flag=1, check with respect to which variable E(.) depends on
% and then specify the hessian considering only such variable.
% In this example E does not depend on any varaible than you may set HE as
% follow

%HE=[];

% If dE.flag=0 at least the entries for the hessian with respect to [x0,u0,uf,xf]
% must be specified
% Here tf and p are not decision variables and they must not be considered.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If db.flag=1  you need to check
% on which variables b(tf,p,x0,u0,xf,uf) depends on
% In this example it depends only on xf and then
% it is necessary to specify only the hessian with respect to xf as follow
...
% If db.flag=0  you need to specify at least the the entries for the hessian with respect to
% [x0,u0,uf,xf]
% Here tf and p are not decision variables and they must not be considered.
