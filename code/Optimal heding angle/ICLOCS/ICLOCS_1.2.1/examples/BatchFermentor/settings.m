...
% Number of nodes for integration in the interval t=[0,tf]; nodes=steps+1
options.nodes=1501; 

% Distribution of integration steps. Set to tau=0 for equispaced steps.
% Otherwise: tau is a vector of length N-1 with 0<tau(i)<1 and sum(tau)=1
options.tau=0;
...
%
% 'idas'     N/S    H    High accuracy, DAE (slow)
%
% 'ode45'    N      M    Most of the time. Try this one first.     
%
% 'ode23'    N      L    For probs with crude error tols or for moderately S problems.
%
% 'ode113'   N      L-H  For probs with stringent error tols or for computationally intensive problems.
%
% 'ode15s'   S      L-M   If ode45 is slow because the prob is stiff.
%
% 'ode23s'   S      L   If using crude error tols to solve stiff  systems and the mass matrix is constant.
%
% 'ode23t'   S      L   For moderately stiff problems if you need a solution without numerical damping.
%
% 'ode23tb'  S      L   If using crude error tols to solve stiff systems.
%
%  Note: 'cvodes' and 'idas' options require the sundialsTB. 
%         Information on the other solvers is taken from the matlab help files. 
...
% Options for forward integration
