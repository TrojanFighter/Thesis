...
\draw [dashed] (A) -- node [below] {$\lambda_r$} ++ (-\pythagwidth, 0);

\draw [dashed] (A) -- node [right] {$\dfrac{\lambda_\theta}{r}$} ++ (0, \pythagheight);

\draw [dashed] (C) -- node [above left]  {$[(\frac{\lambda_\theta}{r})^2 + \lambda_r^2]^\frac{1}{2}$} (B);
